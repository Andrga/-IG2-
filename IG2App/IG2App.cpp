...
void IG2App::setupScene(void) {
	...
	phantomeNode = mSM->getRootSceneNode()->createChildSceneNode();
	SceneNode* sphereNode = phantomeNode->createChildSceneNode();
	sphereNode->attachObject(sphereEnt);
	sphereNode->setPosition(Vector3(1000, 100, 0));
...
//    bbSet->createBillboard({ 0, 50, -50});
	...
	Ogre::SceneNode* npSys = mSM->getRootSceneNode()->createChildSceneNode();
	pSys->setEmitting(true);
	npSys->attachObject(pSys);
}

void IG2App::frameRendered(const Ogre::FrameEvent& evt) {
	Quaternion q = phantomeNode->getOrientation().getRotationTo(Vector3(0,5,0));
	phantomeNode->rotate(q);
}
