...
Intro::Intro(SceneManager* SM, SceneNode* sNode, SceneNode* cNode, OgreBites::Label* lbl, OgreBites::TextBox* txtB)
	: sMang(SM), introNode(sNode), label(lbl), textBox(txtB)
{
...
void Intro::setUpScene(SceneNode* cNode)
{
	...
	// UI
	label->setCaption("Pulsa S para empezar");
	textBox->clearText();
	textBox->appendText("Esperando para empezar");
