...
Labyrinth::Labyrinth(string root, SceneNode* sNode, SceneManager* sCMan, SceneNode* cNode, OgreBites::Label* lbl, OgreBites::TextBox* txtB, IG2App* ig2)
	: gameNode(sNode), camNode(cNode), sMang(sCMan), ig2app(ig2), label(lbl), textBox(txtB)
{
	...
	//-------------CREACION DE LA UI------------//
	label->setCaption("Stage 1");
	textBox->clearText();
	textBox->appendText(" Lives: " + to_string(lives) + "\nPoints: " + to_string(getPoints()));
}
